...
public class SearchSessionTests : TestBase
{
    ...
    {
        // Arrange
        Query query = Query.ParseSearchText("cats")!;
        ...
        mock.Count(Arg.Any<Query>()).Returns(Task.FromResult((int?)5));
        ...
        Assert.AreEqual(Query.ParseSearchText("cats"), session.CurrentQuery);
    ...
    public async Task TestFetchData()
    {
        ...
        ISearchService mock = Substitute.For<ISearchService>();
        mock.LoadIds(query, Arg.Any<int>(), Arg.Any<int>()).Returns(Task.FromResult(idCollection));
        mock.Count(Arg.Any<Query>()).Returns(Task.FromResult((int?)50));
